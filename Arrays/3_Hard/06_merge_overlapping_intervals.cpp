...
// Note-
// 1)Time Complexity: O(N*logN) + O(2*N), where N = the size of the given array.
...
// 2)Space Complexity: O(N), as we are using an answer list to store the merged intervals. Except for the answer array, we are not using any extra space.
...
// Note-
// 1)Time Complexity: O(N*logN) + O(N), where N = the size of the given array.
